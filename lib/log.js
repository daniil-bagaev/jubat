        ...
        log (msg, opts) {
            ...
            opts = 
                (!Object.keys(opts).length) ? 
                    logger.config.opts :
                (opts.type && logger.config.types.hasOwnProperty(opts.type)) ? 
                    Object.keys(logger.opts).forEach(key => { 
                        let types = logger.config.types[opts.type]; 
                        opts[key] = (logger.config.opts[key] !== types[key] && types[key]) ? types[key] : logger.config.opts[key]
                    }) : 
                {
                    type: 'custom',
                    logdir: path.resolve((opts.logdir || logger.config.opts.logdir)),
